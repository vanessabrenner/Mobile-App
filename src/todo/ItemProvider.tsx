...
const reducer: (state: BooksState, action: ActionProps) => BooksState = (state, { type, payload }) => {
  switch (type) {
    ...
    case SAVE_BOOK_SUCCEEDED:
      ...
      const index = updatedBooks.findIndex(b => b.id === savedBook.id);
      if (index === -1) {
        updatedBooks.push(savedBook); // New book added
      } else {
        updatedBooks[index] = savedBook; // Existing book updated
      }
      return { ...state, books: updatedBooks, saving: false };
